...
const validateFile = (file) => {
  ...
  if (!fileType) {
    return {
      valid: false,
      error:
        "Tipo de archivo no permitido. Solo se aceptan: imágenes, videos, PDFs y audio",
    };
  }
  ...
  if (fileSize > config.maxSize) {
    return {
      valid: false,
      error: `El archivo ${fileType} excede el tamaño máximo permitido de ${
...
const uploadFile = async (file, options = {}) => {
  try {
    ...
    if (file.buffer) {
      ...
      fileSource = `data:${file.mimetype};base64,${file.buffer.toString(
        "base64"
      )}`;
    } else if (file.path) {
...
const uploadMultipleFiles = async (files, options = {}) => {
  ...
  if (fileTypes.size > 1) {
    throw {
      success: false,
      error: `Solo se puede subir un tipo de archivo a la vez. Se detectaron: ${Array.from(
        fileTypes
      ).join(", ")}`,
...
/**
 * Actualiza un archivo existente en Cloudinary
 * Elimina el archivo anterior y sube el nuevo
 * @param {string} oldPublicId - ID público del archivo a reemplazar
 * @param {Object} newFile - Nuevo archivo a subir
 * @param {Object} options - Opciones de subida
 * @returns {Promise<Object>} - Resultado de la actualización
 */
const updateFile = async (oldPublicId, newFile, options = {}) => {
  try {
    ...
    // Determinar el resourceType del archivo anterior
    const oldResourceType = options.oldResourceType || config.resourceType;

    // Intentar eliminar el archivo anterior
    let deleteResult = null;
    if (oldPublicId) {
      try {
        deleteResult = await deleteFile(oldPublicId, oldResourceType);
      } catch (deleteError) {
    ...
    // Si se proporciona keepPublicId, mantener el mismo ID
    const uploadOptions = {
      ...options,
      overwrite: options.keepPublicId ? true : options.overwrite || false,
    };

    // Si se quiere mantener el mismo publicId
    if (options.keepPublicId && oldPublicId) {
      // Extraer solo el nombre sin la carpeta si existe
      const publicIdParts = oldPublicId.split("/");
      const fileName = publicIdParts[publicIdParts.length - 1];
      uploadOptions.customOptions = {
        ...uploadOptions.customOptions,
    ...
    const uploadResult = await uploadFile(newFile, uploadOptions);
...
/**
 * Reemplaza un archivo existente manteniendo el mismo publicId
 * @param {string} publicId - ID público del archivo a reemplazar
 * @param {Object} newFile - Nuevo archivo a subir
 * @param {Object} options - Opciones de subida
 * @returns {Promise<Object>} - Resultado del reemplazo
 */
const replaceFile = async (publicId, newFile, options = {}) => {
  try {
    // Validar el nuevo archivo
    const validation = validateFile(newFile);
    if (!validation.valid) {
    ...
    // Opciones para sobrescribir manteniendo el mismo publicId
    const uploadOptions = {
      folder: options.folder,
      resource_type: config.resourceType,
      public_id: publicId,
      overwrite: true,
      invalidate: true, // Invalida el caché de CDN
    ...
    let fileSource;
    if (newFile.buffer) {
      fileSource = `data:${newFile.mimetype};base64,${newFile.buffer.toString(
        "base64"
      )}`;
    } else if (newFile.path) {
      fileSource = newFile.path;
    } else if (newFile.tempFilePath) {
      fileSource = newFile.tempFilePath;
    } else {
      throw new Error("Formato de archivo no soportado");
    }

    // Subir sobrescribiendo
    const result = await cloudinary.uploader.upload(fileSource, uploadOptions);
    ...
    throw {
      success: false,
      error: error.message || "Error al reemplazar el archivo",
      details: error,
    };
  }
};
